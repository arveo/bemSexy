...
class ControllerCheckoutGuestShipping extends Controller {
  	public function index() {	
		...
		if (isset($this->session->data['guest']['shipping']['number'])) {
			$this->data['number'] = $this->session->data['guest']['shipping']['number'];			
		} else {
			$this->data['number'] = '';
		}

		if (isset($this->session->data['guest']['shipping']['complement'])) {
			$this->data['complement'] = $this->session->data['guest']['shipping']['complement'];			
		} else {
	...
	public function validate() {
		...
		if (!$json) {		
			...
			if ($country_info && $country_info['postcode_required'] && (utf8_strlen($this->request->post['postcode']) < 9) || (utf8_strlen($this->request->post['postcode']) > 9)) {
