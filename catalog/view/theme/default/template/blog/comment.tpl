   ...
   <?php foreach ($comments as $comment) { ?>
      ...
      <li class="<?php echo $class; ?>">
         <div id="comment-<?php echo $comment['comment_id'];?>" itemprop="comment" itemscope itemtype="http://schema.org/UserComments">
            ...
            <div class="name"><?php if($comment['website']) { echo '<a href="'.$comment['website'].'"><span itemprop="creator">'.$comment['name'].'</span></a>'; } else { echo $comment['name']; }?></div>
            <div class="created">
         ...
         <?php if ($comment['replyComments']) { ?>
            ...
            <?php foreach ($comment['replyComments'] as $replyComment) { ?>
               ...
               <li class="<?php echo $class2; ?>">
                  ...
                  <div class="name"><?php if($replyComment['website']) { echo '<a href="'.$replyComment['website'].'">'.$replyComment['name'].'</a>'; } else { echo $replyComment['name']; }?></div>
                  <div class="created">
