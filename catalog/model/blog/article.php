...
class ModelBlogArticle extends Model {
   ...
   public function getArticles($data = array()) {
      ...
      if (!$article_data) {
         ...
         if (!empty($data['exclude_category'])) {
            ...
            $categories = explode(',',$data['exclude_category']);
         ...
         if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
            ...
            if (!empty($data['filter_name'])) {
               ...
               foreach ($words as $word) {
                  $implode[] = "LCASE(ad.title) LIKE '%" . $this->db->escape(mb_strtolower($word, 'UTF-8')) . "%' OR LCASE(ad.description) LIKE '%" . $this->db->escape(mb_strtolower($word, 'UTF-8')) . "%'";         
               }
            ...
            if (!empty($data['filter_tag'])) {
               ...
               foreach ($words as $word) {
                  $implode[] = "LCASE(at.tag) LIKE '%" . $this->db->escape(mb_strtolower($data['filter_tag'], 'UTF-8')) . "%' AND at.language_id = '" . (int)$this->config->get('config_language_id') . "'";
               }
   ...
   public function getCategoriesByArticle($article_id) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_article_to_category a2c LEFT JOIN " . DB_PREFIX . "blog_category_description cd ON (a2c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "blog_category c ON (a2c.category_id = c.category_id) WHERE a2c.article_id = '" . (int)$article_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c.status = '1'");

      return $query->rows;
   }
   
   public function getRelatedArticle($article_id) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_article_related ar LEFT JOIN " . DB_PREFIX . "blog_article_description ad ON (ar.related_id = ad.article_id) WHERE ar.article_id = '" . (int)$article_id . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "'");
   ...
   public function getRelatedArticleAtProduct($product_id) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_product_related WHERE product_id = '" . (int)$product_id . "'");
   ...
   public function getComment($comment_id) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_comment c LEFT JOIN " . DB_PREFIX . "blog_article_description ad ON (c.article_id = ad.article_id) WHERE comment_id = '" . (int)$comment_id . "'");
      
      return $query->row;
   }
   
   public function getComments ($data = array()) {
      $sql = "SELECT * FROM " . DB_PREFIX . "blog_comment c WHERE c.status = '1'";
   ...
   public function getCustomerGroup($customer_id) {
      if($this->tableCheck('customer_group', 'name')) {
         ...
         $query = $this->db->query("SELECT cgd.customer_group_id as groupId, cgd.name as groupName FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE c.customer_id = '" . (int)$customer_id . "'");
      }
   ...
   public function getTags($limit = 20) {
      ...
      if($this->tableCheck('product_description', 'tag')) { // if exist mean >= 1.5.4
         ...
         $blogTags      = $this->db->query("SELECT tag, Count(tag) AS total FROM " . DB_PREFIX . "blog_article_tag bat LEFT JOIN " . DB_PREFIX . "blog_article ba ON ba.article_id = bat.article_id WHERE ba.status = '1' AND bat.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY tag ORDER BY RAND() LIMIT " . (int)$limit);
   ...
   public function getArchive($group) {
      ...
      $sql .= " FROM " . DB_PREFIX . "blog_article WHERE status = '1'";
   ...
   public function getArchiveArticle($group, $total = 0, $data = array()) {
      ...
      if ($total) { //assuming only year input use this
         $sql .= " a.article_id FROM " . DB_PREFIX . "blog_article a LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id) WHERE a.status = '1' AND YEAR(a.created) = " . $group . "";
      } else {
         $sql .= " a.article_id FROM " . DB_PREFIX . "blog_article a LEFT JOIN " . DB_PREFIX . "blog_article_to_category a2c ON (a.article_id = a2c.article_id) WHERE a.status = '1' AND MONTHNAME(a.created) = '" . $group . "'";
      }
