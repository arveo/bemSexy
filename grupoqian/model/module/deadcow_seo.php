...
class ModelModuleDeadcowSEO extends Model {
    ...
    public function generateTags($template, $source_langcode) {
        ...
        foreach ($products as $product) {
            ...
            $finalKeywords = array_filter(array_unique($finalKeywords));
            $this->db->query("UPDATE " . DB_PREFIX . "product_description SET tag = '' where product_id = " . (int)$product['product_id'] . " and language_id = " . (int)$product['language_id']);
            ...
            foreach ($finalKeywords as $keyword) {
				...
				$this->db->query("UPDATE " . DB_PREFIX . "product_description SET tag = CONCAT(tag, ',',  '" . $this->db->escape($keyword) . "') where product_id = " . (int)$product['product_id'] . " and language_id = " . (int)$product['language_id']);
            }
        }
    }
    ...
    function _transliteration_replace($ord, $unknown = '?', $langcode = NULL) {
        ...
        if (!isset($map[$bank][$langcode])) {
            ...
            if (file_exists($file)) {
                include $file;
                if ($langcode != 'pt-br' && isset($variant[$langcode])) {
