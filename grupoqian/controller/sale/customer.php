...
class ControllerSaleCustomer extends Controller { 
	...
	public function approve() {
  	...
  	private function getForm() {
		...
		if (isset($this->error['address_complement'])) {
			$this->data['error_address_complement'] = $this->error['address_complement'];
		} else {
			$this->data['error_address_complement'] = '';
		}
    	...
    	if (isset($this->request->post['company_id'])) {
			...
			$this->data['company_id'] = $customer_info['company_id'];
		} else {
    	...
    	if (isset($this->request->post['ie'])) {
      		$this->data['ie'] = $this->request->post['ie'];
		} elseif (!empty($customer_info)) { 
			$this->data['ie'] = $customer_info['ie'];
		} else {
      		$this->data['ie'] = '';
    	}
	
    	if (isset($this->request->post['rg'])) {
      		$this->data['rg'] = $this->request->post['rg'];
		} elseif (!empty($customer_info)) { 
  	...
  	private function validateForm() {
		...
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {
				...
				if ((utf8_strlen($value['number']) < 2) || (utf8_strlen($value['number']) > 128)) {
					$this->error['address_number'][$key] = $this->language->get('error_number');
				}
			    
				if ((utf8_strlen($value['address_2']) < 3) || (utf8_strlen($value['address_2']) > 128)) {
					$this->error['address_address_2'][$key] = $this->language->get('error_address_2');
				}
				...
				if ($country_info) {
					if ($country_info['postcode_required'] && (utf8_strlen($value['postcode']) < 9) || (utf8_strlen($value['postcode']) > 9)) {
